...
const Payment = () => {
  ...
  const updatePayment = async (registrationNo) => {
    try{
      const res = await axios.post('http://localhost:5001/updateStatus', {updatePayStatus, registrationNo})
      if (res.status === 200) 
      {
  ...
  const searchData = async () => {
    try {
      const res = await axios.post('http://localhost:5001/search', searchPaymentData)
      setpayment(res.data)
    } catch(err)
    {
      console.log(err)
