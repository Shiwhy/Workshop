...
const InvoiceView = (props) => {
  ...
  useEffect(() => {
    axios.get('http://localhost:5000/generatedInvoices')
    .then((res) => {
  ...
  const searchJobcard =  async() => {
    try{
      const res = await axios.post('http://localhost:5001/search', searchInvoice)
      setInvoiceData(res.data)
    }catch(err){
      console.log(err)
    }
  ...
  const clearSearch = async () => {
    try{
        const res = await axios.get('http://localhost:5000/generatedInvoices')
        setInvoiceData(res.data)
